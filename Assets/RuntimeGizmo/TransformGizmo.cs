...
namespace RuntimeGizmos
{
	...
	public class TransformGizmo : MonoBehaviour
	{
		...
		Coroutine activeTransformCoroutine;
		...
		{
			if(activeTransformCoroutine != null)
			{
				StopCoroutine(activeTransformCoroutine);
				activeTransformCoroutine = null;
			}
		...
		public void AddTarget(Transform target, bool addCommand = true)
		{
			if(target != null)
			{
				...
				if(onTargetSelected != null) onTargetSelected(target);
			}
		}
		...
		public void RemoveTarget(Transform target, bool addCommand = true)
		{
			if(target != null)
			{
				...
				if(onTargetDeselected != null) onTargetDeselected(target);
			}
		}

		List<Transform> clearedTargetsBuffer = new List<Transform>();
		...
		public void ClearTargets(bool addCommand = true)
		{
			...
			clearedTargetsBuffer.Clear();
			clearedTargetsBuffer.AddRange(targetRootsOrdered);
			...
			if(clearedTargetsBuffer.Count > 0)
			{
				for(int i = 0; i < clearedTargetsBuffer.Count; i++)
				{
					if(onTargetDeselected != null) onTargetDeselected(clearedTargetsBuffer[i]);
				}
		...
		void ForceUpdatePivotPointOnChange()
		{
			...
			if(hasChanged && !isTransforming && !suppressHoverCallbacks)
			{
				if(axis != Axis.None)
				{
					if(onGizmoHover != null) onGizmoHover(type, axis);
				}
				else
				{
